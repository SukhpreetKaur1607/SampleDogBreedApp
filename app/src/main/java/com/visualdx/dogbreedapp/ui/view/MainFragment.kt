...
class MainFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvAutoComplete.setOnItemClickListener { parent, _, position, _ ->
            val selectedBreed = parent.adapter.getItem(position) as DogSubBreed
            viewModel.setSelection(selectedBreed)
            binding.tvAutoComplete.setText("")

            if (viewModel._subBreedName.value.isNullOrEmpty()) viewModel.getRandomImagesByBreed() else viewModel.getRandomImages()
